...
# Select variables
demo_char_table1 <- df %>%
  select(arm, age_range, gender, raceethnicity_recoded, phq_b_total, ucla_b_total, trt_dosage)

# Ensure arm is a factor
demo_char_table1$arm <- factor(demo_char_table1$arm, levels = c("Intervention", "Control"))

# Separate categorical vs continuous variables
categorical_vars <- c("age_range", "gender", "raceethnicity_recoded", "trt_dosage")
continuous_vars <- c("phq_b_total", "ucla_b_total")

# 1. Summarize categorical variables as N (%)
