...
## Data Extraction, Transformation, and final Load for Analysis
          ...
          # Load files and establish environment NOTE - Data is on external drive
...
# Create Calculated fields (Age, Race/Ethnicity, PHQ-9, UCLA - LS- 20, and creating Dosages as a function of Attendance in Invention Tx arm only)
          ...
          # Attendance (bins) - prepare for factor levels of "treatment dosage" with discrete labels and continuous
          ...
          df$attend_s1f <- as.factor(df$attend_s1)
          # Calculate frequencies
          attend_f_test1 <- table(df$attend_s1f)
          # Create the bar plot
          barplot(attend_f_test1, main="Attendance", 
                  xlab="Sessions 1", ylab="Frequency", col="purple")
          
          # Convert a variable to a factor
          df$attend_s2f <- as.factor(df$attend_s2)
          # Calculate frequencies
          attend_f_test2 <- table(df$attend_s2)
          # Create the bar plot
          barplot(attend_f_test2, main="Attendance", 
                  xlab="Sessions 2", ylab="Frequency", col="purple")
          # Convert a variable to a factor
          df$attend_s3f <- as.factor(df$attend_s3)
          # Calculate frequencies
          attend_f_test3 <- table(df$attend_s3f)
          # Create the bar plot
          barplot(attend_f_test3, main="Attendance", 
                  xlab="Sessions 3", ylab="Frequency", col="purple")
          
          # Convert a variable to a factor
          df$attend_s4f <- as.factor(df$attend_s4)
          # Calculate frequencies
          attend_f_test4 <- table(df$attend_s4f)
          # Create the bar plot
          barplot(attend_f_test4, main="Attendance", 
                  xlab="Sessions 4", ylab="Frequency", col="purple")
          
          # Convert a variable to a factor
          df$attend_s5f <- as.factor(df$attend_s5)
          # Calculate frequencies
          attend_f_test5 <- table(df$attend_s5f)
          # Create the bar plot
          barplot(attend_f_test5, main="Attendance", 
                  xlab="Sessions 5", ylab="Frequency", col="purple")
          
          # Convert a variable to a factor
          df$attend_s6f <- as.factor(df$attend_s6)
          
          # Calculate frequencies
          attend_f_test6 <- table(df$attend_s6f)
          # Create the bar plot
          barplot(attend_f_test6, main="Attendance", 
                  xlab="Sessions 6", ylab="Frequency", col="purple")
          
          
          # Dosage Total as CONTINUOUS
          ...
          # Dosage Category as DISCRETE- Sample label that should be vetted only for the pilot/skills assessment
          ...
          df$dosage_prop <- (rowSums(df[, 95:100] == "yes", na.rm = TRUE) + #treats yes as full dosage, partial as half dosage, no as missed dosage
                               0.5 * rowSums(df[, 95:100] == "partial",
                                             na.rm = TRUE)) / bh_dosage_total
...
# Suggested: Make a Table 1 (version is for internal but include total and use data suppression for small Ns)
          ...
          # Table 1 Var
          demo_char_table1 <- df %>%
            select(arm, age_range, gender, raceethnicity_recoded, phq_b_total, 
                   phq_3_total, phq_6_total, ucla_b_total, ucla_3_total, 
                   ucla_6_total,trt_dosage, trt_dosage_total, income)
          
          # Level with intervention arm
          demo_char_table1$arm <- factor(demo_char_table1$arm, 
                                         levels = c("Intervention", "Control"))
          
          # Prep diff var types
          categorical_vars <- c("age_range", "gender", "raceethnicity_recoded",
                                "trt_dosage")
          continuous_vars <- c("phq_b_total", "phq_3_total","phq_6_total",
                               "ucla_b_total", "ucla_3_total","ucla_6_total",
                               "income", "trt_dosage_total")
          
          # Cat variables as N (%)
          cat_table <- demo_char_table1 %>%
            select(arm, all_of(categorical_vars)) %>%
            pivot_longer(cols = -arm, names_to = "characteristic", 
                         values_to = "value") %>%
            filter(!is.na(value)) %>%
            ...
            pivot_wider(names_from = arm, values_from = n_perc, 
                        values_fill = "0 (0%)") %>%
          ...
          # Final Table - I always like to keep common R code in a template or repository with documentation for quick tables and analysis
          draft_demo_table %>%
            gt() %>%
...
hist(df$income)
 ...
 income_plot <- ggplot(df, aes(x = income, fill = arm, color = arm)) +
  ...
  theme_classic()
#rq(outcome ~ arm + age + gender + income, data = df, tau = 0.5) # not completed
