---
title: "Practice Large Dataset Analysis"
...
```{r Health Outcomes ONLY, warning=FALSE, echo=FALSE}
#Selected Chronic Health Outcomes (DATASOURCE CDC in 2024)
# Filter to PLACES subset
...
health_outcomes_table <- health_outcomes_summary %>%
  ...
  tab_header(
    ...
    subtitle = "CDC PLACES Indicators — County Level (2024)"
  )
...
health_outcome_plots <- health_outcomes_numeric %>%
  ...
  theme_minimal() +
  labs(title = "Distribution of Health Outcomes Measures Version 1 (2024)")


#Boxplot
  ggplot(health_outcomes_numeric %>% 
           pivot_longer(everything(), names_to="measure", values_to="value"),
         aes(x = measure, y = value)) +
    geom_boxplot() +
    coord_flip() +
    theme_minimal()+
  labs(title = "Distribution of Health Outcomes Measures Version 2 (2024)")
...
```{r Outcomes, warning=FALSE, echo=FALSE}
###-----------------------------------Complete workflow
summarize_category <- function(df, cat) {
  df %>%
    filter(Category == cat) %>%
    select(where(is.numeric)) %>%
    summarise(across(
...
# Examples:
prevention_summary      <- summarize_category(combined_data, "Prevention")
disability_summary      <- summarize_category(combined_data, "Disability")
health_outcomes_summary <- summarize_category(combined_data, "Health Outcomes")


#Plot Prevention 
ggplot(combined_data, aes(x = summarize_category)) +
  geom_histogram(bins = 30) +
  theme_minimal() +
...
#Option 1
model1 <- lm(summarize_category ~ phys_nf_prim_care_pc_exc_rsdt_22, data = combined_data)
summary(model1)

#Option 2 2020 Census  AI/AN, Black, NHPI,65+ in poverty
model2 <- lm(summarize_category ~ phys_nf_prim_care_pc_exc_rsdt_22 + 
               popn_aian_pct_20 + popn_bl_pct_20 + popn_nhpi_pct_20 +
               popn_hsp_pct_20 + pers_deep_povty_ge65_pct_22 + rural_urban_contnm_23, #attempt - need better flag for rurality
             data = combined_data)
